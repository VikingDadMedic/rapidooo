...
/**
 * @api {PUT} /api/page/:archive_id/publish Archive page
 * @apiDescription Archive page
 * @apiGroup Page
 * @apiVersion 0.1.0
 * @apiPermission Admin
 ...
 * HTTP/1.1 200 Ok
 * {
 *		"message": "settings_pages__page_archived"
 * }
 * @apiUse Error40X
 * @apiUse Error50X
 */
...
func Archive(c *gin.Context) {
	...
	if payload.AccessLevel != jwt.AdminAL && obj.PageCollaboration.HasAuthor(pageID) && !obj.PageCollaboration.IsAuthor(pageID, payload.ID) {
		c.AbortWithStatusJSON(http.StatusForbidden, map[string]interface{}{"message": "settings_pages__cannot_archive_page"})
		return
	}
	...
	if pageID == settings.HomePage() {
		c.AbortWithStatusJSON(http.StatusForbidden, map[string]interface{}{"message": "settings_pages__cannot_archive_home_page"})
		return
	}

	// Archives page
	obj.Page.Published = 0
	if err := obj.Page.Update(); err != nil {
		log.Println(err)
		return
	}

	c.JSON(http.StatusOK, map[string]interface{}{"message": "settings_pages__page_archived"})
}
